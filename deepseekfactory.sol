...
interface ICozySwapPair {
    ...
    // ✅ FIXED: Hapus functions yang sudah ada di ERC20
    ...
    // ✅ Hanya functions yang unique untuk Pair
...
interface ICozySwapCallee {
    function cozySwapCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}

// ✅ FIXED: Interface ERC20 yang proper
interface ICozySwapERC20 {
    event Approval(address indexed owner, address indexed spender, uint value);
    event Transfer(address indexed from, address indexed to, uint value);
...
contract CozySwapERC20 is ICozySwapERC20 {
    // ✅ FIXED: Gunakan private/internal variables, public getters
    string private constant _name = 'CozySwap LPs';
    string private constant _symbol = 'COZY-LP';
    ...
    // ✅ FIXED: Explicit getter functions
...
// ✅ FIXED: Pair contract implements both interfaces
contract CozySwapPair is ICozySwapPair, CozySwapERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'CozySwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'CozySwap: K');
        }
...
contract CozySwapFactory is ICozySwapFactory {
