...
contract CozyStakingComplete {
    ...
    function enterBoosting(uint256 _cozyAmount) external {
        ...
        emit BoostEntered(msg.sender, _cozyAmount, boostMultiplier[msg.sender]);
    }
    ...
    function _updateBoostMultiplier(address _user) internal {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            if (userInfo[i][_user].amount > 0) {
                userInfo[i][_user].boostMultiplier = newBoost;
            }
        }
    }
    ...
    function getUserInfo(address _user) external view returns (
        ...
        currentBoost = cozyStakedForBoost[_user] > 0 ? 
            BASE_MULTIPLIER + ((cozyStakedForBoost[_user] * boostRate) / (100 * 1e18)) : 
            BASE_MULTIPLIER;
    }
